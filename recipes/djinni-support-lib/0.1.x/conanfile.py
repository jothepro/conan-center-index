...
class DjinniSuppotLib(ConanFile):
    ...
    def python_support(self):
        if self.options.target == "auto":
            return self.settings.os not in ["iOS", "watchOS", "tvOS", "Android"]
        else:
    ...
    def _configure_cmake(self):
        ...
        if self.jni_support:
            self._cmake.definitions["DJINNI_JNI_WITH_MAIN"] = self.options.jni_with_main
